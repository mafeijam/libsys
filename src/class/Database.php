...
class Database
{
	...
	protected $defaultConfigFile = DB_CONFIG;
	...
	protected function parseConfig($setting)
	{
		...
		if (is_file($setting)) {
			...
			$config = config($this->defaultConfigFile);
		}
	...
	public function all($key = null, $order = 'asc')
	{
		if (isset($key)) {
			return $this->select()->orderBy($key, $order)->get();
		}
	...
	{
		return $this->select()->orderById('desc')->limit($limit)->get();
	}
	...
	{
		return $this->select($key)->where($key, $value)->count();
	}
	...
	public function relate($table, $id)
	{
		return $this->table($table)->whereId($id)->first();
	}
